...
- Die Windows-GUI nutzt Microsoft Edge WebView2 für das HTML-Dashboard. Auf aktuellen Windows-10/11-Systemen ist die Runtime meist bereits vorhanden.
...
```text
build\installer\UptimeKumaTrayAgent-Setup-1.1.2-x64.msi
build\installer\UptimeKumaTrayAgent-Setup-1.1.2-x86.msi
```
...
```text
build\installer\UptimeKumaTrayAgent-Setup-1.1.2.exe
```
...
```text
build/uptime-kuma-agent-1.1.2-linux-x64.tar.gz
build/uptime-kuma-agent-1.1.2-linux-arm64.tar.gz
```
...
```bash
tar -xzf uptime-kuma-agent-1.1.2-linux-x64.tar.gz
cd uptime-kuma-agent-1.1.2-linux-x64
