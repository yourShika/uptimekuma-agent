...
Ab Version 1.0.9 nutzt die Windows-App eine Dashboard-Oberfläche mit Sidebar-Navigation, KPI-Karten, Monitor-Suche, Statusfilter, eigener Einstellungsseite und weiterhin allen bisherigen Funktionen für Ping-, TCP-, Dienst-, Laufwerks-, Log- und Update-Workflows.
...
```text
build\installer\UptimeKumaTrayAgent-Setup-1.0.9-x64.msi
build\installer\UptimeKumaTrayAgent-Setup-1.0.9-x86.msi
```
...
```text
build\installer\UptimeKumaTrayAgent-Setup-1.0.9.exe
```
...
```text
build/uptime-kuma-agent-1.0.9-linux-x64.tar.gz
build/uptime-kuma-agent-1.0.9-linux-arm64.tar.gz
```
...
```bash
tar -xzf uptime-kuma-agent-1.0.9-linux-x64.tar.gz
cd uptime-kuma-agent-1.0.9-linux-x64
