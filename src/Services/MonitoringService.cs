...
public sealed class MonitoringService
{
    ...
    private readonly TcpConnectionMonitor _tcpConnectionMonitor = new();
    private readonly DriveMonitorService _driveMonitorService = new();
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var serviceName in check.RestartServicesOnFailure)
        {
            ...
            try
            {
                var restart = await _windowsServiceManager
                    .RestartServiceAsync(
                        serviceName,
    ...
    private async Task<MonitorCheckResult> ExecutePingAsync(ScheduledCheck check, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var reply = await ping
                .SendPingAsync(check.Host, check.TimeoutMs)
                .WaitAsync(TimeSpan.FromMilliseconds(check.TimeoutMs), cancellationToken)
                .ConfigureAwait(false);
            ...
            if (reply.Status == IPStatus.Success)
            {
                return MonitorCheckResult.Up(
                    AppendMachineInfo(check, $"Ping OK: {check.Name} erreichbar"),
                    Math.Max(1, reply.RoundtripTime));
            }

            return MonitorCheckResult.Down(
                BuildDownSinceMessage(check, $"Ping Fehler: {check.Name} nicht erreichbar"),
                MapPingStatus(reply.Status),
                stopwatch.ElapsedMilliseconds);
        }
        catch (TimeoutException)
        {
            return MonitorCheckResult.Down($"Ping Fehler: {check.Name} Timeout", "Ping fehlgeschlagen", stopwatch.ElapsedMilliseconds);
        }
        catch (PingException ex) when (FindException<SocketException>(ex) is { } socket)
        {
            return MonitorCheckResult.Down($"Ping Fehler: {check.Name} {socket.Message}", MapSocketErrorForPing(socket), stopwatch.ElapsedMilliseconds);
        }
        catch (SocketException ex)
        {
            return MonitorCheckResult.Down($"Ping Fehler: {check.Name} {ex.Message}", MapSocketErrorForPing(ex), stopwatch.ElapsedMilliseconds);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            return MonitorCheckResult.Down($"Ping Fehler: {check.Name} {ex.Message}", "Ping fehlgeschlagen", stopwatch.ElapsedMilliseconds);
        }
    }
    ...
    {
        var statusResult = _windowsServiceManager.GetStatus(check.ServiceName);
